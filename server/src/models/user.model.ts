...
const UserSchema = new Schema(
  {
    name: { type: String, required: true, minlength: 2, maxlength: 120, trim: true, index: true },
    email: { type: String, required: true, unique: true, lowercase: true, trim: true, index: true },
    password: { type: String, minlength: 6, select: false },
    oauthProvider: { type: String, enum: ['email', 'google', 'discord'], default: 'email' },
    googleId: { type: String, sparse: true, index: true },
    role: { type: String, enum: ['member', 'moderator', 'admin'], default: 'member', index: true },
    avatarUrl: String,
...
UserSchema.index({ 'gamification.points': -1, completedTaskCount: -1 })
...
UserSchema.statics.findByDiscordId = async function (discordId: string): Promise<UserDocument | null> {
  return this.findOne({ 'discord.id': discordId })
}

UserSchema.statics.findByDiscordIdWithTokens = async function (discordId: string): Promise<UserDocument | null> {
  return this.findOne({ 'discord.id': discordId }).select('+discord.accessToken +discord.refreshToken')
}

UserSchema.statics.findByGoogleId = async function (googleId: string): Promise<UserDocument | null> {
  return this.findOne({ 'google.id': googleId }).select('+google.accessToken +google.refreshToken')
}

UserSchema.statics.findByGoogleIdWithTokens = async function (googleId: string): Promise<UserDocument | null> {
  return this.findOne({ 'google.id': googleId }).select('+google.accessToken +google.refreshToken')
}
