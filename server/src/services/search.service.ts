import { User, Project, Comment, File, Resource } from '@/models';
...
class SearchService {
  async searchProjects(query: string, page: number, limit: number): Promise<SearchResults> {
    try {
      ...
      const results: SearchResult[] = projects.map(project => ({
        ...
        relevance: this.calculateRelevance(query, String(project.title || ""), String(project.description || ""))
      }));
      ...
      throw new InternalServerError('PROJECTS_SEARCH_FAILED');
    }
  }
  ...
  async searchMembers(query: string, page: number, limit: number): Promise<SearchResults> {
    try {
      ...
      const results: SearchResult[] = users.map(user => ({
        ...
        description: String(user.bio || user.email || ""),
        type: 'member',
        url: `/users/${String(user._id || "")}`,
        relevance: this.calculateRelevance(query, String(user.name || ""), String(user.bio || user.email || ""))
      }));
      ...
      throw new InternalServerError('MEMBERS_SEARCH_FAILED');
    }
  }
  ...
  async searchFiles(query: string, page: number, limit: number): Promise<SearchResults> {
    try {
      ...
      const results: SearchResult[] = files.map(file => ({
        ...
        type: 'file',
        url: `/files/${String(file._id || "")}`,
        relevance: this.calculateRelevance(query, String(file.name || ""))
      }));
      ...
      throw new InternalServerError('FILES_SEARCH_FAILED');
    }
  }
  ...
  async searchComments(query: string, page: number, limit: number): Promise<SearchResults> {
    try {
      ...
      const results: SearchResult[] = comments.map(comment => ({
        ...
        type: 'comment',
        url: `/${(comment as any).project ? 'projects' : 'tasks'}/${(comment as any).project?._id || (comment as any).task?._id}#comment-${String(comment._id || "")}`,
        relevance: this.calculateRelevance(query, String(comment.content || ""))
      }));
      ...
      throw new InternalServerError('COMMENTS_SEARCH_FAILED');
    }
  }
  ...
  async searchResources(query: string, page: number, limit: number): Promise<SearchResults> {
    try {
      ...
      const results: SearchResult[] = resources.map(resource => ({
        ...
        relevance: this.calculateRelevance(query, String(resource.title || ""), String(resource.description || ""))
      }));
      ...
      throw new InternalServerError('RESOURCES_SEARCH_FAILED');
    }
  }
  ...
  async globalSearch(query: string, page: number, limit: number): Promise<SearchResults> {
    try {
      ...
      throw new InternalServerError('GLOBAL_SEARCH_FAILED');
    }
  }
